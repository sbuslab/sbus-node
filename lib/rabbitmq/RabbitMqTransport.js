...
class RabbitMqTransport {
  constructor(config) {
    this.config = config;
    this.config.queueConfigs = config.queueConfigs || {};
  ...
  getQueueConfig(name) {
    const cfg = this.config.queueConfigs[name] || null;

    return {
      name: cfg.name || name,
      durable: cfg.durable || false,
      exclusive: cfg.exclusive || false,
      autodelete: cfg.autodelete || false,
      exchange: cfg.exchange || this.commonExchange.name,
      exchangeType: cfg.exchangeType || this.commonExchange.exchangeType,
      routingKey: cfg.routingKey || name,
    };
  }
    ...
    return amqp.connect(this.rabbitmqConnection)
      .then((conn) => {
        ...
        return this._channelEstablish(
          ...
          (ch) => { this.channel = ch; },
          () => this._channelEstablish(this.connection),
          (resp) => {
            ...
            return this.channel.consume(this.replyToQueue, (msg) => {
              ...
              // eslint-disable-next-line consistent-return
              return this.promiseStorage[msg.properties.correlationId].resolve_ex(message).then((res) => {
  ...
  _channelEstablish(connection, routingKey = null, queryOptions = {}, saveChannel, channelOnError, assertQueueCallback) {
    ...
    return connection.createChannel().then((ch) => {
      ...
      ch.on('error', err => channelOnError(err, ch));

      // Создаем очередь с routing key и выполняем callback
      return ch.assertQueue(routingKey, queryOptions).then(() => assertQueueCallback(ch));
    });
  }


  send(routingKey, msg, context = { Headers: {} }, transportOptions = {}) {
    ...
    const propsBldr = {
      ...
      expiration: context.timeout ? context.timeout : (hasResponse ? this.defaultTimeout : null),
  ...
  subscribe(routingKey, handler) {
    ...
    }, ch => ch.bindQueue(routingKey, this.options.exchange, routingKey).then(() =>
      // eslint-disable-next-line consistent-return
