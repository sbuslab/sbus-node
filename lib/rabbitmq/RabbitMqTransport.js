...
class RabbitMqTransport {
  constructor(config) {
    ...
    this.channelReplyErrorCallback = config.channelReplyErrorCallback || (err => ({ status: 500, body: { message: err.toString() } }));
  }
        ...
        return Promise
          ...
          .catch((e) => {
            if (this.channelReplyErrorCallback) {
              this.channelReplyErrorCallback(e);
            } else {
              throw e;
            }
          })
          .then((res) => {
            if (originalMsg.properties.replyTo) {
              this.log(`sbus resp ~~~> ${routingKey}: ${res}`);
          ...
          .catch((e) => {
            this.log(`Unhandled error when process message. ${e.toString()}`);
          });
      }, { noAck: true })));
  }
