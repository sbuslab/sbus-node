...
class RabbitMqTransport {
        ...
        return this._channelEstablish(
          ...
          (resp) => {
            ...
            return this.channel.consume(this.replyToQueue, (msg) => {
              ...
              this.logger.info(`sbus resp <~~~ ${handler.routingKey}: ${JSON.stringify(message)}`);
  ...
  send(routingKey, msg, context = {}, transportOptions = {}) {
    ...
    this.logger.info(`sbus ~~~~> ${routingKey}: ${JSON.stringify(msg)}`);
  ...
  subscribe(routingKey, handler, context) {
    const _this = this;
    const exchange = context.exchange ? { ...this.commonExchange, name: context.exchange } : this.commonExchange;
    return this._channelEstablish(this.connection, routingKey, exchange, { durable: false }, (ch) => {
      ...
      ch.consume(routingKey, (msg) => {
        ...
        _this.logger.info(`sbus <~~~ ${routingKey}: ${JSON.stringify(msg)}`);

        return Promise
          .resolve(msg)
          .then(msg => handler(msg, originalMsg.properties) || (() => ({})))
          .catch((e) => {
            _this.logger.error(`Sbus error: ${e.stack}`);
            if (_this.channelReplyErrorCallback) {
              _this.channelReplyErrorCallback(e);
            } else {
              throw e;
            }
          })
          .then((res) => {
            if (originalMsg.properties.replyTo) {
              _this.logger.info(`sbus resp ~~~> ${routingKey}: ${JSON.stringify(res)}`);
          ...
          .catch((e) => {
            _this.logger.info(`Unhandled error when process message. ${e.toString()}`);
          });
      }, { noAck: true })));
  }
