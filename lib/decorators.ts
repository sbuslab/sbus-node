...
export function schedule(period: string) {
  ...
  return function (target: any, propertyKey: string, descriptor: InitMethodDescriptor) {
    ...
    INITED_SUBSCRIPTIONS.get(target.constructor.name)?.map((sub) => {
      if (sub.propertyKey === propertyKey) {
        sub.schedulePeriod = moment.duration(parseInt(duration[0], 10), duration[1] as DurationInputArg2).asMilliseconds();
      }
      return sub;
    });
  };
